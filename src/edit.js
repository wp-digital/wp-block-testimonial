...
const TestimonialText = ({ value, onChange }) => (
	<RichText
		...
		multiline="p"
		value={value}
		placeholder={__('Text', 'wpd-blocks')}
		onChange={onChange}
...
export default function Edit(props) {
	...
	return (
		...
		>
			<InspectorControls>
				<PanelBody title={__('Author Image', 'wpd-blocks')} initialOpen>
						...
						{!!attachmentId && (
							<MediaUploadCheck>
								<Button
									...
									isDestructive
								>
									{__('Remove Author Image', 'wpd-blocks')}
								</Button>
							</MediaUploadCheck>
						)}
					</div>
				...
				<PanelBody
					title={__('Block settings', 'wpd-blocks')}
					initialOpen={false}
				>
					<PanelRow>
						<ToggleControl
							label={__('Show additional info', 'wpd-blocks')}
					...
					<PanelRow>
						<RadioControl
							label={__('Text layout', 'wpd-blocks')}
							selected={layout}
							options={[
								{
									label: __('Top', 'wpd-blocks'),
									value: LAYOUT_TEXT_TOP,
								},
								{
									label: __('Bottom', 'wpd-blocks'),
									value: LAYOUT_TEXT_BOTTOM,
								},
								{
									label: __('Right', 'wpd-blocks'),
			...
			<figcaption className={`${BLOCK_CLASS_NAME}-author`}>
					...
					<div className={`${BLOCK_CLASS_NAME}-author__image`}>
						...
						<MediaUploadCheck>
							<Button
								onClick={onAuthorImageRemove}
								icon="dismiss"
								title={__('Remove Author Image', 'wpd-blocks')}
								aria-label={__(
									'Remove Author Image',
									'wpd-blocks'
								)}
				...
				<div className={`${BLOCK_CLASS_NAME}-author__main`}>
					...
					<RichText
						...
						value={authorName}
						placeholder={__('Author name', 'wpd-blocks')}
